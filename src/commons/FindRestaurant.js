...
export default class FindRestaurant extends Component {
  ...
  doSearchResults = async () => {
    ...
    this.state.registeredRestaurants.forEach (res => {
      let availableString = `${res.restaurantName} ${res.location ? res.location.addressName : ''}  ${res.priceRange ? String (res.priceRange.minimum) + String (res.priceRange.maximum) : ''}`;
      if (
        String (availableString).toLowerCase ().indexOf (stringToSearch) >= 0 &&
